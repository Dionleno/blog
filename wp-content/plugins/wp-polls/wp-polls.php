...
add_action('wp_ajax_get_pollsq_all', 'vb_get_pollsq_all');
add_action('wp_ajax_nopriv_get_pollsq_all', 'vb_get_pollsq_all');
function vb_get_pollsq_all(){
    global $wpdb;
    $response = $res = array();
    $list = $wpdb->get_results("SELECT * FROM $wpdb->pollsq WHERE pollq_active = 1");
    ...
    foreach ($list as $key => $value) {
         $res[$key]->text = $value->pollq_question;
         $res[$key]->value = $value->pollq_id;
...
function poll_scripts_admin($hook_suffix) {
	$poll_admin_pages = array('wp-polls/polls-manager.php', 'wp-polls/polls-add.php', 'wp-polls/polls-options.php', 'wp-polls/polls-templates.php', 'wp-polls/polls-uninstall.php','post.php');
